...
aws ec2 run-instances \
    ...
    --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=Romen_CLU}]' \
    --user-data file:///home/ec2-user/userdata.sh
```

- To see the each instances Ip we\'ll use describe instance CLI command
```bash
aws ec2 describe-instances --filters "Name=tag:Name,Values=Romen_CLU"
```

- You can run the query to find Public IP and instance_id of instances:
```bash
aws ec2 describe-instances --filters "Name=tag:Name,Values=Romen_CLU" --query 'Reservations[].Instances[].PublicIpAddress[]'

aws ec2 describe-instances --filters "Name=tag:Name,Values=Romen_CLU" --query 'Reservations[].Instances[].InstanceId[]'
```

- To delete instances
```bash
aws ec2 terminate-instances --instance-07ebc0ac47423cc74 <We can learn this with querry>
```
